...
getSupportedPackages()
{
    ...
    #List all spec files in the <installer root>/<distro> directory
       for SpecFile in `ls "${AbsDistroDir}"/*.NoX.spec 2>/dev/null`; do
...
buildPackage()
{
	...
	X_DIR=`ls "${AbsDistroDir}"/x*.${X_NAME}.NoX.spec`
	...
	PKG_SPEC="${AbsDistroDir}/${X_DIR}.NoX.spec"
	...
	# Removing all libraries from temp folder which are not nedded
		rm -rf ${TmpDrvFilesDir}/etc/security > /dev/null
		rm -rf ${TmpDrvFilesDir}/usr/include > /dev/null
		rm -rf ${TmpDrvFilesDir}/usr/share/applications > /dev/null
		rm -rf ${TmpDrvFilesDir}/usr/share/ati > /dev/null
		rm -rf ${TmpDrvFilesDir}/usr/share/doc/amdcccle > /dev/null
		rm -rf ${TmpDrvFilesDir}/usr/share/icons > /dev/null
		rm -rf ${TmpDrvFilesDir}/usr/X11R6/lib > /dev/null
		rm -rf ${TmpDrvFilesDir}/usr/lib/fglrx > /dev/null
		rm ${TmpDrvFilesDir}/usr/sbin/{amdnotifyui,atieventsd} > /dev/null
		rm -rf ${TmpDrvFilesDir}/usr/share/ > /dev/null

		if [ "${ARCH}" = "x86_64" ]; then
		   	rm -rf ${TmpDrvFilesDir}/usr/X11R6/lib64/fglrx > /dev/null
			rm -rf ${TmpDrvFilesDir}/usr/X11R6/lib64 > /dev/null
			rm -rf ${TmpDrvFilesDir}/usr/X11R6/lib64/modules > /dev/null
			rm -rf ${TmpDrvFilesDir}/usr/lib64/fglrx > /dev/null
		fi

        # Move the binaries to /usr/bin, soon we need to add aticonfig too
        mkdir -p ${TmpDrvFilesDir}/usr/bin
	mv ${TmpDrvFilesDir}/usr/X11R6/bin/{amd-console-helper,atiodcli} \
		    ${TmpDrvFilesDir}/usr/bin
	rm -rf ${TmpDrvFilesDir}/usr/X11R6/bin/*
...
END_SED_SCRIPT
        ...
        ### amdconfig to symlink to aticonfig, soon we need to uncomment it ###
